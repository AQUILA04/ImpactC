...
export default function HomeScreen() {
  ...
  const [profiles, setProfiles] = useState<Array<{ id: string; firstName: string; age: number; profession: string; city: string; tagline: string; profilePhotoUrl?: string; profilePhotoThumbUrl?: string | null }>>([]);
  const [journeyId, setJourneyId] = useState('');
  const [message, setMessage] = useState('');
  const [chat, setChat] = useState<Array<{ id: string; content: string; sentAt: string }>>([]);
  const [chatBlocked, setChatBlocked] = useState(false);
  const [uploadingPhoto, setUploadingPhoto] = useState(false);
  const [selectedPhoto, setSelectedPhoto] = useState<{ name: string; reference: string } | null>(null);
  ...
  const photoUri = (reference: string, variant: 'original' | 'thumbnail' = 'thumbnail') => {
    if (!reference.startsWith('media://profile/')) return reference;
    const id = reference.replace('media://profile/', '').split('/')[0];
    return `${API}/media/profile/${id}${variant === 'thumbnail' ? '/thumbnail' : ''}`;
  };
  ...
  </ScrollView><Modal visible={Boolean(selectedPhoto)} transparent animationType="fade" onRequestClose={() => setSelectedPhoto(null)}><View style={styles.imageModal}><Pressable accessibilityRole="button" accessibilityLabel="Fermer la photo" onPress={() => setSelectedPhoto(null)} style={StyleSheet.absoluteFill} /><View style={styles.imageModalContent}><Text style={styles.imageModalTitle}>{selectedPhoto ? `Photo de ${selectedPhoto.name}` : ''}</Text>{selectedPhoto ? <Image accessibilityLabel={`Photo originale de ${selectedPhoto.name}`} source={{ uri: photoUri(selectedPhoto.reference, 'original'), headers: token ? { authorization: `Bearer ${token}` } : undefined }} style={styles.originalImage} resizeMode="contain" /> : null}<Pressable accessibilityRole="button" onPress={() => setSelectedPhoto(null)} style={styles.secondary}><Text style={styles.secondaryText}>Fermer</Text></Pressable></View></View></Modal><View style={styles.nav}>{nav.map(([key, label]) => <Pressable key={key} onPress={() => { if (key === 'discover') void loadDiscover(); else if (key === 'chat' && journeyId) void loadChat(); else setTab(key); }} style={styles.navButton}><Text style={[styles.navText, tab === key && styles.navTextActive]}>{label}</Text></Pressable>)}</View></SafeAreaView>;
}

const styles = StyleSheet.create({ safe: { flex: 1, backgroundColor: '#F8F7F4' }, header: { backgroundColor: '#3B5998', padding: 20 }, brand: { color: '#F8F7F4', fontSize: 12, fontWeight: '700', letterSpacing: 3 }, title: { color: '#fff', fontSize: 26, fontWeight: '700', marginTop: 4 }, notice: { margin: 12, padding: 12, borderWidth: 1, borderColor: '#C9A84C', backgroundColor: '#fff9e8', borderRadius: 10 }, noticeText: { color: '#856404', fontSize: 14 }, body: { padding: 16, paddingBottom: 92 }, card: { backgroundColor: '#fff', borderRadius: 16, padding: 18, gap: 12, shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 8, elevation: 2 }, heading: { color: '#2C4270', fontSize: 24, fontWeight: '700' }, copy: { color: '#4b5563', lineHeight: 21 }, field: { gap: 5 }, label: { color: '#1F2937', fontWeight: '600', fontSize: 14 }, input: { borderWidth: 1, borderColor: '#cbd5e1', borderRadius: 10, minHeight: 48, paddingHorizontal: 12, backgroundColor: '#fff' }, multiline: { minHeight: 90, textAlignVertical: 'top' }, primary: { minHeight: 48, justifyContent: 'center', alignItems: 'center', borderRadius: 10, backgroundColor: '#3B5998', paddingHorizontal: 14 }, primaryText: { color: '#fff', fontWeight: '700' }, secondary: { minHeight: 48, justifyContent: 'center', alignItems: 'center', borderRadius: 10, borderWidth: 1, borderColor: '#3B5998', paddingHorizontal: 14 }, secondaryText: { color: '#2C4270', fontWeight: '700' }, consent: { flexDirection: 'row', gap: 10, alignItems: 'center', paddingVertical: 6 }, check: { fontSize: 24, color: '#3B5998' }, empty: { alignItems: 'center', gap: 8, padding: 28 }, profileCard: { marginTop: 14, borderWidth: 1, borderColor: '#e5e7eb', borderRadius: 14, padding: 14, gap: 7 }, uploadPreview: { width: '100%', aspectRatio: 4 / 5, borderRadius: 12, backgroundColor: '#E5E7EB' }, photo: { width: '100%', aspectRatio: 4 / 5, alignItems: 'center', justifyContent: 'center', backgroundColor: '#E5E7EB', borderRadius: 12, overflow: 'hidden' }, photoImage: { width: '100%', height: '100%' }, photoText: { color: '#2C4270', fontSize: 58, fontWeight: '700' }, profileName: { fontSize: 18, fontWeight: '700', color: '#1F2937' }, tagline: { fontStyle: 'italic', color: '#4b5563' }, slot: { borderRadius: 10, padding: 14, backgroundColor: '#eff6ff' }, warning: { borderWidth: 1, borderColor: '#EF4444', backgroundColor: '#EF44441A', padding: 12, borderRadius: 10, gap: 4 }, warningTitle: { color: '#b91c1c', fontWeight: '700' }, warningText: { color: '#7f1d1d', lineHeight: 20 }, message: { alignSelf: 'flex-start', borderRadius: 12, backgroundColor: '#EFEFEF', padding: 10, gap: 4 }, time: { color: '#64748b', fontSize: 11 }, errorInput: { borderColor: '#EF4444', borderWidth: 2 }, errorText: { color: '#b91c1c', fontSize: 13, fontWeight: '600' }, disabled: { backgroundColor: '#94a3b8' }, imageModal: { flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(15, 23, 42, 0.78)', padding: 20 }, imageModalContent: { width: '100%', maxWidth: 480, borderRadius: 16, backgroundColor: '#fff', padding: 16, gap: 12 }, imageModalTitle: { color: '#2C4270', fontSize: 18, fontWeight: '700' }, originalImage: { width: '100%', aspectRatio: 4 / 5, borderRadius: 12, backgroundColor: '#E5E7EB' }, nav: { flexDirection: 'row', borderTopWidth: 1, borderColor: '#e5e7eb', backgroundColor: '#fff', paddingVertical: 8 }, navButton: { flex: 1, minHeight: 42, alignItems: 'center', justifyContent: 'center' }, navText: { color: '#64748b', fontSize: 12, fontWeight: '600' }, navTextActive: { color: '#3B5998' } });
