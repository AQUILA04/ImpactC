...
export default function HomeScreen() {
  ...
  const [form, setForm] = useState({ firstName: '', lastName: '', gender: 'FEMALE', dateOfBirth: '1998-01-01', city: '', churchDepartment: '', departmentLeader: '', profession: '', financialRange: '', profilePhotoUrl: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330', tagline: '', searchMinAge: '25', searchMaxAge: '40', consent: false });
  ...
  return <SafeAreaView style={styles.safe}><View style={styles.header}><Text style={styles.brand}>IMPACTC</Text><Text style={styles.title}>Parcours supervisé</Text></View><View accessibilityLiveRegion="polite" style={styles.notice}><Text style={styles.noticeText}>{notice}</Text></View><ScrollView contentContainerStyle={styles.body}>
    ...
    {tab === 'onboarding' && <View style={styles.card}><Text style={styles.heading}>Inscription en 3 étapes</Text><Text style={styles.copy}>Les champs sont validés avant envoi. Vous devez avoir 18 ans révolus et accepter le traitement des données.</Text>{field('Prénom', 'firstName')}{field('Nom', 'lastName')}{field('Date de naissance (AAAA-MM-JJ)', 'dateOfBirth')}{field('Ville', 'city')}{field('Département de l’église', 'churchDepartment')}{field('Responsable de département', 'departmentLeader')}{field('Profession', 'profession')}{field('Fourchette financière', 'financialRange')}{field('Phrase d’accroche', 'tagline')}{field('URL de photo 4:5', 'profilePhotoUrl')}<Pressable onPress={() => setForm((current) => ({ ...current, gender: current.gender === 'FEMALE' ? 'MALE' : 'FEMALE' }))} style={styles.secondary}><Text style={styles.secondaryText}>Genre : {form.gender === 'FEMALE' ? 'Femme' : 'Homme'}</Text></Pressable><Pressable onPress={() => setForm((current) => ({ ...current, consent: !current.consent }))} style={styles.consent}><Text style={styles.check}>{form.consent ? '✓' : '○'}</Text><Text style={styles.copy}>J’accepte le traitement de mes données pour ce parcours.</Text></Pressable><Pressable onPress={submitProfile} style={styles.primary}><Text style={styles.primaryText}>Soumettre à validation</Text></Pressable></View>}
