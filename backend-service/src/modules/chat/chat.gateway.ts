...
} from '../../common/services/frontend-origins';
...
export class ChatGateway implements OnGatewayConnection {
  ...
  async handleConnection(client: Socket): Promise<void> {
    try {
      ...
      if (!isAllowedFrontendOrigin(origin)) throw new Error('forbidden origin');
  ...
  async join(
    ...
    @MessageBody() body: { journeyId: string },
  ): Promise<void> {
  ...
  async send(
    @ConnectedSocket() client: Socket,
    @MessageBody() body: { journeyId: string; content: string },
  ): Promise<void> {
    try {
      ...
      if (result.blocked) {
        client.emit('chat:error', {
          code: 'ANTI_CONTACT_BLOCKED',
          message: result.reason,
        });
        return;
      }
      this.server
        .to(`journey:${body.journeyId}`)
        .emit('message:receive', result.message);
      client.emit('message:sent', result.message);
    } catch {
      client.emit('chat:error', {
        code: 'CHAT_ACCESS_DENIED',
        message: 'This Journey chat is not available.',
      });
    }
  }
}
