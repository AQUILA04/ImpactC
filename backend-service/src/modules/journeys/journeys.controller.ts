...
export class JourneysController {
  ...
  decision(
    ...
    @Body() body: { partner: 1 | 2; decision: ConsentDecision },
  ) {
    return this.journeys.recordDecision(
      user.sub,
      journeyId,
      body.partner,
      body.decision,
  ...
  step2(
    ...
    return this.journeys.promoteToStep2(user.sub, journeyId);
  }
  ...
  step3(
    ...
    return this.journeys.promoteToStep3(user.sub, journeyId);
  }
  ...
  step4(
    ...
    return this.journeys.promoteToStep4(user.sub, journeyId);
  }
  ...
  terminate(
    ...
    return this.journeys.terminate(user.sub, journeyId, body.reason);
  }

  @Get('journeys/kanban')
  board(@CurrentUser() user: JwtPayload) {
    return this.journeys.board(user.sub);
  }

  @Get('dashboard')
  dashboard(@CurrentUser() user: JwtPayload) {
    return this.journeys.dashboard(user.sub);
  }

  @Post('internal/journeys/check-expirations')
  @Roles(UserRole.ADMIN)
  check() {
    return this.journeys.checkExpirations();
  }
}
