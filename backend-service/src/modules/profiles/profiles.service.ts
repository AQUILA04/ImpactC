...
export type ProfileInput = {
  ...
  consent: boolean;
  nationality?: string;
  churchTenureMonths?: number;
  activitySector?: string;
  personalDescription?: string;
  spiritualCriteria?: string;
  professionalCriteria?: string;
};
...
export class ProfilesService {
  ...
  async createOrResubmit(userId: string, input: ProfileInput) {
    ...
    const existing = await this.prisma.celibataireProfile.findUnique({
      where: { userId },
    });
    const editableStatuses: ProfileStatus[] = [
      ProfileStatus.REJECTED,
      ProfileStatus.PENDING_VALIDATION,
    ];
    if (existing && !editableStatuses.includes(existing.status)) {
      throw new BadRequestException(
        'Only pending or rejected profiles can be submitted',
      );
    }
    const data = {
      ...input,
    ...
    const profile = existing
      ? await this.prisma.celibataireProfile.update({
          where: { id: existing.id },
          data,
        })
      : await this.prisma.celibataireProfile.create({
          data: { ...data, userId },
        });
    await this.audit.record(
      userId,
      existing ? 'PROFILE_RESUBMITTED' : 'PROFILE_SUBMITTED',
      'CelibataireProfile',
      profile.id,
    );
    return profile;
  ...
  async me(userId: string) {
    const profile = await this.prisma.celibataireProfile.findUnique({
      where: { userId },
      include: {
        availabilitySlots: {
  ...
  async updateAvailability(
    ...
    if (!profile) throw new NotFoundException('Profile not found');
    if (profile.status !== ProfileStatus.CELIBATAIRE_LIBRE)
      throw new ForbiddenException(
        'Only approved available members can update availability',
      );
    ...
    await this.prisma.$transaction([
      ...
      this.prisma.availabilitySlot.createMany({
        data: slots.map((slot) => ({ ...slot, profileId: profile.id })),
      }),
    ]);
    await this.audit.record(
      userId,
  ...
  async moderate(
    ...
    if (!profile) throw new NotFoundException('Profile not found');
    if (profile.status !== ProfileStatus.PENDING_VALIDATION)
      throw new BadRequestException('Profile is not pending moderation');
    if (decision === 'reject' && !note?.trim())
      throw new BadRequestException('A rejection reason is required');
    const status =
      decision === 'approve'
        ? ProfileStatus.CELIBATAIRE_LIBRE
        : ProfileStatus.REJECTED;
    ...
    await this.prisma.$transaction([
      this.prisma.notification.create({
        data: {
          ...
          type:
            decision === 'approve' ? 'PROFILE_APPROVED' : 'PROFILE_REJECTED',
          title:
            decision === 'approve'
              ? 'Profile approved'
      ...
      this.prisma.auditLog.create({
        data: {
          ...
          action:
            decision === 'approve' ? 'PROFILE_APPROVED' : 'PROFILE_REJECTED',
          targetType: 'CelibataireProfile',
          targetId: profileId,
          metadata: { note: note ?? null },
        },
      }),
    ]);
    await this.businessNotifications.profileModerated({
      profileId: updated.id,
      userEmail: updated.user.email,
      firstName: updated.firstName,
      decision,
      updatedAt: updated.moderatedAt ?? new Date(),
  ...
  async discover(userId: string, cursor?: string, take = 12) {
    const viewer = await this.prisma.celibataireProfile.findUnique({
      where: { userId },
    });
    if (!viewer || viewer.status !== ProfileStatus.CELIBATAIRE_LIBRE)
      throw new ForbiddenException(
        'Discovery is available only to approved members',
      );
    const profiles = await this.prisma.celibataireProfile.findMany({
      where: {
        ...
        id: { not: viewer.id },
        NOT: [
          { journeysAsOne: { some: { status: 'ACTIVE' } } },
          { journeysAsTwo: { some: { status: 'ACTIVE' } } },
        ],
      },
    ...
    return {
      items: profiles.map((profile) => ({
        ...profile,
        age: this.age(profile.dateOfBirth),
      })),
  ...
  private validateProfile(input: ProfileInput): void {
    const required = [
      ...
      input.lastName,
      input.city,
      input.churchDepartment,
      input.departmentLeader,
      input.profession,
      input.financialRange,
      input.profilePhotoUrl,
      input.tagline,
    ];
    if (required.some((value) => !value?.trim()))
      throw new BadRequestException(
        'All required profile fields must be completed',
      );
    if (!input.consent)
    ...
    if (
      !input.searchMinAge ||
      !input.searchMaxAge ||
      input.searchMinAge < 18 ||
      input.searchMaxAge < input.searchMinAge
    )
      throw new BadRequestException('Search age range is invalid');
    const dob = new Date(input.dateOfBirth);
    if (Number.isNaN(dob.getTime()) || this.age(dob) < 18)
      throw new BadRequestException('You must be at least 18 years old');
    if (/^media:\/\/profile\/[a-f0-9-]{36}$/i.test(input.profilePhotoUrl))
      return;
    try {
      const url = new URL(input.profilePhotoUrl);
      const allowedHosts = (
        process.env.PROFILE_MEDIA_ALLOWED_HOSTS ??
      ...
      throw new BadRequestException(
        'A valid HTTPS profile photo URL from an approved media host is required',
      );
    }
  }

  private thumbnailReference(profilePhotoUrl: string): string | null {
    return /^media:\/\/profile\/[a-f0-9-]{36}$/i.test(profilePhotoUrl)
