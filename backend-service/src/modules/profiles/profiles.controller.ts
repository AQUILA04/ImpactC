...
export class ProfilesController {
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles(UserRole.CELIBATAIRE)
  availability(@CurrentUser() user: JwtPayload, @Body() body: { slots: Array<{ weekday: number; startTime: string; endTime: string }> }) {
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles(UserRole.CELIBATAIRE)
  discover(@CurrentUser() user: JwtPayload, @Query('cursor') cursor?: string, @Query('take') take?: string) {
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles(UserRole.RESPONSABLE, UserRole.ADMIN)
  moderate(@CurrentUser() user: JwtPayload, @Param('profileId') profileId: string, @Body() body: { decision: 'approve' | 'reject'; note?: string }) {
    return this.profiles.moderate(user.sub, profileId, body.decision, body.note);
  }
