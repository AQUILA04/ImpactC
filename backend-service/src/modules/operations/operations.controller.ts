...
export class OperationsController {
  ...
  create(
    ...
    @Body() body: { title: string; content: string; coupleNames: string },
  ) {
    if (
      !body.title?.trim() ||
      !body.content?.trim() ||
      !body.coupleNames?.trim()
    )
      throw new Error(
        'Testimonial title, content and couple names are required',
      );
    return this.prisma.testimonial.create({
      data: {
        ...
        coupleNames: body.coupleNames.trim(),
      },
    });
  }
